...
#include "lio_utility.h"

#include <just_slam/associator.h>
#include <just_slam/optimizer.h>
...
public:
  ...
  ros::ServiceClient optimize_clt, associate_clt;
  ...
  mapOptimization() {
    ...
    optimize_clt = nh.serviceClient<just_slam::optimizer>("/pose_optimize");
    associate_clt =
        nh.serviceClient<just_slam::associator>("/feature_associate");
...
#pragma omp parallel for num_threads(numberOfCores)
  ...
  bool LMOptimization(int iterCount) {
    ...
    // cope with coordinate transformation lidar <- camera
    ...
    std::cout << "LIO-SAM method cost: "
              << sqrt(pow(deltaR, 2) + pow(deltaT, 2)) << std::endl;
  ...
  void scan2MapOptimization() {
    ...
    if (laserCloudCornerLastDSNum > edgeFeatureMinValidNum &&
      ...
      just_slam::optimizer optimize_srv;
      just_slam::associator associate_srv;

      for (int iterCount = 0; iterCount < 30; iterCount++) {
        laserCloudOri->clear();
        coeffSel->clear();

        PointTypePose p6_in = trans2PointTypePose(transformTobeMapped);
        associate_srv.request.xyzypr_guess = {
            p6_in.x, p6_in.y, p6_in.z, p6_in.yaw, p6_in.pitch, p6_in.roll};

        pcl::toROSMsg(*laserCloudCornerLastDS,
                      associate_srv.request.src_sharp_cloud);
        pcl::toROSMsg(*laserCloudSurfLastDS,
                      associate_srv.request.src_flat_cloud);
        pcl::toROSMsg(*laserCloudCornerFromMapDS,
                      associate_srv.request.map_sharp_cloud);
        pcl::toROSMsg(*laserCloudSurfFromMapDS,
                      associate_srv.request.map_flat_cloud);

        associate_clt.call(associate_srv);
        // pcl::PointCloud<PointType>::Ptr mine_fpoints(
        // new pcl::PointCloud<PointType>());
        // pcl::PointCloud<PointType>::Ptr mine_coeffs(
        // new pcl::PointCloud<PointType>());
        // pcl::fromROSMsg(associate_srv.response.points, *mine_fpoints);
        // pcl::fromROSMsg(associate_srv.response.coeffs, *mine_coeffs);
        std::cout << "Mine method fpoints number: "
                  << associate_srv.response.points.width << std::endl;
        std::cout << "Mine method coeffs number: "
                  << associate_srv.response.coeffs.width << std::endl;
        ...
        // pcl::toROSMsg(*laserCloudOri, optimize_srv.request.points);
        // pcl::toROSMsg(*coeffSel, optimize_srv.request.coeffs);
        optimize_srv.request.points = associate_srv.response.points;
        optimize_srv.request.coeffs = associate_srv.response.coeffs;
        optimize_srv.request.xyzypr_in = {p6_in.x,   p6_in.y,     p6_in.z,
                                          p6_in.yaw, p6_in.pitch, p6_in.roll};
        optimize_srv.request.is_first_iteration = (iterCount == 0);
        optimize_clt.call(optimize_srv);

        // TODO: check iteration over return true at some point!!!
        const auto &po = optimize_srv.response.xyzypr_out;
        std::cout << "Mine method cost: " << optimize_srv.response.final_cost
                  << std::endl;
        std::cout << "Mine method result: " << po[0] << ", " << po[1] << ", "
                  << po[2] << ", [" << po[3] << ", " << po[4] << ", " << po[5]
                  << "]" << std::endl;
        std::cout << "-------------" << std::endl;

        if (LMOptimization(iterCount) == true) {
          std::cout << "LIO-SAM method fpoints number: "
                    << laserCloudOri->points.size() << std::endl;
          std::cout << "LIO-SAM method coeffs number: "
                    << coeffSel->points.size() << std::endl;
          std::cout << "-------------" << std::endl;

          PointTypePose p6_out = trans2PointTypePose(transformTobeMapped);
          std::cout << "LIO-SAM method result: " << p6_out.x << ", " << p6_out.y
                    << ", " << p6_out.z << ", [" << p6_out.yaw << ", "
                    << p6_out.pitch << ", " << p6_out.roll << "]" << std::endl;
          std::cout << "= # " << iterCount
                    << "==================================" << std::endl;
          break;
        }
        std::cout << "LIO-SAM method fpoints number: "
                  << laserCloudOri->points.size() << std::endl;
        std::cout << "LIO-SAM method coeffs number: " << coeffSel->points.size()
                  << std::endl;
        std::cout << "-------------" << std::endl;
  ...
  void updatePath(const PointTypePose &pose_in) {
    ...
    tf::Quaternion q = tf::Quaternion(pose_in.roll, pose_in.pitch, pose_in.yaw);
